...
博客仍由 GitHub Pages 承载，文章仍是仓库里的 Markdown。CloudBase 只运行一个 HTTP 云函数：验证发布密码后，使用保存在函数环境变量中的 GitHub Fine-grained Token，将文章 Markdown 和 `static/js/article-index.js` 原子提交到 `master`。
...
tcb fn deploy blog-publisher --httpFn --path /blog-publisher -e <你的环境ID>
```

也可以在 CloudBase 控制台创建「HTTP 云函数」，运行时选 Node.js 18 或更高版本，再上传这个目录的代码。目录中的 `scf_bootstrap` 会启动 Node HTTP 服务并监听 CloudBase 要求的 `9000` 端口。

`scf_bootstrap` 必须是 LF 换行且具备可执行权限；仓库的 `.gitattributes` 已固定 LF。若在 Windows 上以 zip 手工上传，请先通过 Git 提交该文件，或在 WSL 中执行 `chmod +x scf_bootstrap` 后再打包。[CloudBase 启动脚本文档](https://docs.cloudbase.net/cloud-function/develop/scf-bootstrap)

为函数配置 HTTP 网关路径，例如 `/blog-publisher`。函数的最终地址形如：

```text
https://<环境相关域名>/blog-publisher
```

HTTP 云函数可被普通 `fetch` 调用；CloudBase 官方也支持用 CLI 的 `--httpFn` 方式部署。[CloudBase HTTP 云函数文档](https://docs.cloudbase.net/service/access-cloud-function) [CloudBase CLI 文档](https://cloud.tencent.com/document/product/876/41539)
...
window.BLOG_PUBLISH_CONFIG = {
  apiBaseUrl: 'https://<环境相关域名>/blog-publisher'
};
```
